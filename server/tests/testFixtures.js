...
const testFixtures = async () => {
	...
	await query(SQL`
        INSERT INTO users (user_id, email, display_name, trusted, admin) VALUES
            (1, 'test@test.com', 'Test Admin', 1, 1),
            (2, 'user@test.com', 'Non-Admin', 1, 0),
            (3, 'untrusted@test.com', 'Untrusted', 0, 0);

    `);

	// token = SHA256 hash of 'admin' (1), 'user' (2), and 'untrusted' (3)
	await query(SQL`
        INSERT INTO sessions (session_id, token, user_id, expires_at) VALUES
            (1, '8c6976e5b5410415bde908bd4dee15dfb167a9c873fc4bb8a81f6f2ab448a918', 1, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 1 DAY)),
