...
describe('DELETE /v1/caselists/{caselist}/schools/{school}/teams/{team}/rounds/{round}', () => {
	beforeEach(async () => {
		...
		await fs.promises.writeFile(
			`${config.UPLOAD_DIR}/testdeleteround.docx`,
			'test',
		);
	});
	...
	it('should delete a round', async () => {
		...
		try {
			await fs.promises.access(
				`${config.UPLOAD_DIR}/testdeleteround-DELETED-v${roundsHistory[0].version}.docx`,
				fs.constants.F_OK,
			);
		} catch (err) {
	...
	afterEach(async () => {
		try {
			...
			files = files.filter((f) => f.startsWith('testdeleteround'));
			await Promise.all(
