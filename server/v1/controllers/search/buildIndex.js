...
export const buildIndex = async (killPool = false, recent = false) => {
	...
	for (const file of files) {
		...
		let meta;
		try {
			const response = await fetch(config.TIKA_META_URL, {
				method: 'PUT',
				headers: {
			...
			meta['dc:subject'] = meta['dc:subject']
				? `${meta['dc:subject']}`.replaceAll('.', '')
				: null;
			meta['meta:keyword'] = meta['meta:keyword']
				? `${meta['meta:keyword']}`.replaceAll('.', '')
				: null;
		} catch (err) {
			solrLogger.error(
				`Failed to extract metadata for ${file.download_path}: ${err.message}`,
			);
			continue;
		}
		...
		let textContents;
		try {
			const text = await fetch(config.TIKA_URL, {
				method: 'PUT',
				headers: {
					Accept: 'text/plain',
				},
			...
			solrLogger.error(
				`Failed to extract text content for ${file.download_path}: ${err.message}`,
			);
			continue;
		}
		...
		const body = JSON.stringify([
			{ ...meta, id: file.download_path, content: textContents },
		]);
		...
		try {
			...
			solrLogger.info(
				`Successfully ingested ${file.download_path} into Solr...`,
			);
		} catch (err) {
			solrLogger.error(
				`Failed to ingest ${file.download_path} into Solr: ${err.message}`,
			);
			continue;
		}
	}
	...
	for (const cite of cites) {
		...
		try {
			...
			solrLogger.error(
				`Failed to ingest cite ${cite.cite_id} into Solr: ${err.message}`,
			);
			continue;
		}
	}
