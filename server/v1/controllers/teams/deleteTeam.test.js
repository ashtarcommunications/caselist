...
describe('DELETE /v1/caselists/{caselist}/schools/{school}/teams/{team}', () => {
	beforeEach(async () => {
		await fs.promises.mkdir(`${config.UPLOAD_DIR}`, { recursive: true });
		await fs.promises.writeFile(
			`${config.UPLOAD_DIR}/testdeleteteam.docx`,
			'test',
		);
	});
	...
	it('should delete a team', async () => {
		...
		const newRound = await query(SQL`
            INSERT INTO rounds (team_id, tournament, side, round, opensource) VALUES
                (${newTeam.insertId}, 'TestDelete', 'A', '1', 'testdeleteteam.docx');
        `);
		await query(SQL`
		...
		const teamsHistory = await query(SQL`
            SELECT * FROM teams_history WHERE team_id = ${newTeam.insertId} AND event <> 'test'
        `);
		assert.strictEqual(teamsHistory.length, 1, 'Team history entry');

		const roundsHistory = await query(SQL`
            SELECT * FROM rounds_history WHERE round_id = ${newRound.insertId} AND event <> 'test'
        `);
		assert.strictEqual(roundsHistory.length, 1, 'Round history entry');

		const citesHistory = await query(SQL`
            SELECT COUNT(*) AS 'count' FROM cites_history WHERE round_id = ${newRound.insertId} AND event <> 'test'
        `);
		...
		try {
			await fs.promises.access(
				`${config.UPLOAD_DIR}/testdeleteteam-DELETED-v${roundsHistory[0].version}.docx`,
				fs.constants.F_OK,
			);
		} catch (err) {
	...
	it('should allow an admin to delete an archived team', async () => {
		await request(server)
			.delete(
				`/v1/caselists/archivedcaselist/schools/archivedschool/teams/archivedteam`,
			)
	...
	afterEach(async () => {
		try {
			...
			files = files.filter((f) => f.startsWith('testdeleteteam'));
			await Promise.all(
