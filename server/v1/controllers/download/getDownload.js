...
export const downloadLimiter = rateLimiter({
	...
	handler: (req, res) => {
		debugLogger.info(
			`10 downloads/1m rate limit enforced on user ${req.user_id}`,
		);
		res.status(429).send({
			message: 'You can only download 10 files per minute. Wait and try again.',
		});
	},
...
export const weeklyLimiter = rateLimiter({
	...
	handler: (req, res) => {
		debugLogger.info(
			`5 downloads/1d rate limit enforced on user ${req.user_id}`,
		);
		res.status(429).send({
			message:
				'You can only download 5 bulk files per day. Wait and try again.',
		});
	},
	...
	GET: async (req, res) => {
		...
		try {
			await fs.promises.access(
				`${config.UPLOAD_DIR}/${req.query.path}`,
				fs.constants.F_OK,
			);
		} catch (err) {
		...
		return res.status(200).download(`${config.UPLOAD_DIR}/${req.query.path}`);
	},
