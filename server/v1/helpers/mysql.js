...
export const query = async (sql) => {
    const maxRetries = typeof config.DB_RETRIES === 'number' && config.DB_RETRIES > -1
        ? config.DB_RETRIES
        : 5;
    const retryDelay = typeof config.DB_RETRY_DELAY === 'number' && config.DB_RETRY_DELAY > -1
    ...
    for (let i = 0; i < maxRetries; i++) {
        try {
            ...
            if (err.code === 'ECONNREFUSED') {
                debugLogger.error(`Database connection error ${err.code}, retry #${i}`, { err, stack: err.stack });
            } else if (err.errno === 1213 || err.errno === 1205) {
                debugLogger.error(`Deadlock error ${err.errno}, retry #${i}`, { err, stack: err.stack });
            } else {
                debugLogger.error(`Query error ${err.errno}`, { err, stack: err.stack });
                queryLogger.error(`Query error ${err.errno}`, { err, stack: err.stack });
                // Exit retries early
