...
export const fetchBase = async (path, options = {}, body = {}) => {
	...
	const fetchOptions = {
		...
		maxRetries: options.method && options.method !== 'GET' ? 1 : 3,
...
export const loadTeamHistory = async (caselist, school, team) => {
	return fetchBase(
		`caselists/${caselist}/schools/${school}/teams/${team}/history`,
	);
};

export const loadRounds = async (caselist, school, team, side) => {
	return fetchBase(
		`caselists/${caselist}/schools/${school}/teams/${team}/rounds?side=${side || ''}`,
	);
};
...
export const loadCites = async (caselist, school, team, side) => {
	return fetchBase(
		`caselists/${caselist}/schools/${school}/teams/${team}/cites?side=${side || ''}`,
	);
};
...
export const downloadFile = async (path) => {
	return fetchBase(`download?path=${encodeURIComponent(path)}`, {
		raw: true,
		maxRetries: 1,
		headers: {},
	});
