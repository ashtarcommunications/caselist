...
const TeamRounds = () => {
	...
	const handleDeleteRound = useCallback(
		async (id) => {
			try {
				toast.dismiss();
				const response = await deleteRound(
					caselist,
					school,
					team,
					parseInt(id),
	...
	const handleDeleteRoundConfirm = useCallback(
		(e) => {
	...
	const handleDeleteCite = useCallback(
		async (id) => {
			try {
				toast.dismiss();
	...
	const handleDeleteCiteConfirm = useCallback(
		(e) => {
			const { id } = e.currentTarget;
			if (!id) {
				return false;
	...
	const handleToggleCites = useCallback(
		(e) => {
	...
	const timestamp = moment(teamData.updated_at, 'YYYY-MM-DD HH:mm:ss').format(
		'l',
	);
	...
	return (
		<div className={isMobile ? styles.mobile : undefined}>
			...
			<h1 className={styles.teamname}>
				...
				{!caselistData.archived && lastNames && `(${lastNames})`}
				{!caselistData.archived && (
					<button
						type="button"
						className={`pure-button ${styles.claim}`}
			...
			{teamData.updated_by && (
				<p className={styles.timestamp}>
					Last updated by{' '}
					{teamData.updated_by ? teamData.updated_by : 'unknown'} on {timestamp}
				</p>
			)}
			...
			<div className={styles.buttons}>
				...
				{!caselistData.archived && (
					<Link
						to={`/${caselist}/${school}/${team}/add`}
						className={styles['add-round']}
					>
