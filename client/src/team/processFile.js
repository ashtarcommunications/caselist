	...
	acceptedFiles.forEach((file) => {
		...
		reader.onload = async () => {
			...
			try {
				...
				setProcessing(false);
				toast.error(
					'File could not be processed for cites. Are you sure this is a Verbatim file?',
				);
				return false;
			}
			if (html.length > 1000000) {
				toast.warn(
					"This looks like a very large file, which may slow down your browser. Are you sure it's a speech document?",
				);
			}
			...
			if (
				...
				setProcessing(false);
				toast.warn(
					'Auto-detecting cites failed - this file appears to have already been wikified in Verbatim! Auto-detection only works on unconverted files.',
				);
				return false;
			}
			...
			for (let i = elements.length - 1; i >= 0; i--) {
				if (
					...
					elements[i - 1].innerHTML =
						`${elements[i - 1].innerHTML}<br>${elements[i].innerHTML}`;
					elements.splice(i, 1);
				}
			}
			...
			const processedHTML = elements.map((e) => e.outerHTML).join('');

			// Split cites on the largest heading level in doc, or block titles by default
			let headerToSplit = '<h3>';
			if (processedHTML.indexOf('<h2>') > -1) {
				headerToSplit = '<h2>';
			}
			if (processedHTML.indexOf('<h1>') > -1) {
				headerToSplit = '<h1>';
			}
			const citeEntries = processedHTML
				.split(headerToSplit)
