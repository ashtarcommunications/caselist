		...
		<div className={styles.cites}>
			{row.row?.original?.citesopen && row.row?.original?.tournament && (
				<p title={`Created ${createdAt}`} className={styles.roundinfo}>
					...
					{row.row?.original?.opponent && (
						<span> vs {row.row?.original?.opponent}</span>
					)}
					{row.row?.original?.judge && (
			...
			>
				...
				<span className={styles.caret}>
					<FontAwesomeIcon
						icon={row.row?.original?.citesopen ? faAngleDown : faAngleUp}
					/>
				</span>
			</h1>
			<span
...
const CitesTable = ({
	...
	const handleCopyCites = useCallback(
		async (id) => {
