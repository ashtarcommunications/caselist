...
vi.mock('react-hook-form', async () => {
	...
	return {
		...
		useWatch: vi
			.fn()
			.mockImplementation(() => [
				{ id: 0, title: 'Title', cites: 'Cites', open: true },
			]),
	};
});
...
describe('CiteEditor', () => {
	it('Renders a cite editor', async () => {
		...
		await waitFor(() =>
			assert.isOk(document.querySelector('textarea'), 'Renders a cite box'),
		);
		await waitFor(() =>
			assert.isOk(
				document.querySelector('input[type="checkbox"]'),
				'Renders an open toggle',
			),
		);

		fireEvent.click(screen.getByTestId('trash'));
		await waitFor(() =>
			assert.strictEqual(mockRemove.mock.calls.length, 1, 'Called remove'),
		);
	});

	it('Renders a warning for old wiki syntax', async () => {
		useWatch.mockImplementation(() => [
			{ id: 0, title: 'Title', cites: '= Cites', open: true },
		]);
		const mockRemove = vi.fn();
		const Component = () => {
