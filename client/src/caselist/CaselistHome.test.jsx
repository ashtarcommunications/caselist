...
describe('CaselistHome', () => {
	it('Renders a caselist homepage', async () => {
		render(<CaselistHome />);
		await waitFor(() =>
			assert.isOk(screen.queryByText('Test Caselist'), 'Heading exists'),
		);
		await waitFor(() =>
			assert.isOk(document.querySelector('iframe'), 'Tutorial video exists'),
		);
	});
	...
	it('Renders an archived warning', async () => {
		...
		render(<CaselistHome />);
		await waitFor(() =>
			assert.isOk(screen.queryAllByText('archived'), 'Archived message exists'),
		);
