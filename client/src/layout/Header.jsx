...
const Header = () => {
	...
	let message =
		'You must be a real student, judge, or coach on Tabroom to post on the caselist. ';
	message +=
		'If you think this determination is in error, try logging out and back in. If your ';
	message +=
		'account is brand new, try waiting a few days and logging in again. ';
	message +=
		'Until then, your account is in read-only mode, so you can browse, but not post.';

	return (
		<header
			className={`${styles.header} ${styles[className]} ${isMobile ? styles.mobile : ''}`}
		>
			<h1>
				<Link to="/">openCaselist</Link>
			</h1>
			{auth.user?.loggedIn && (caselist || location.includes('openev')) && (
				<SearchForm />
			)}
