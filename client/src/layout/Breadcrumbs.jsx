...
const Breadcrumbs = () => {
	...
	const openev = location.startsWith('/openev');
	...
	return (
		<div className={styles.breadcrumbs}>
			...
			{tag && (
				<Link to={`/openev/${year}/${tag}`}>
					<span> / {campAbbreviations[tag] || tagAbbreviations[tag]}</span>
				</Link>
			)}

			{caselist && (
				<Link to={`/${caselist}`}>
					<span> / {caselistData.name}</span>
				</Link>
			)}
			{school && (
				<Link to={`/${caselist}/${school}`}>
					<span> / {schoolData.display_name}</span>
				</Link>
			)}
			{team && (
