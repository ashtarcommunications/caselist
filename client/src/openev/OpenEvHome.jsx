...
const OpenEvHome = () => {
	...
	if (openEvFiles.message) {
		return (
			<Error
				statusCode={openEvFiles.statusCode}
				message={openEvFiles.message}
			/>
		);
	}
	...
	return (
		<div>
			...
			{!tag && (
				<>
					...
					<p>
						...
						Anyone can download these files, free of charge. Use them to teach
						debate, compete, or learn about the topic.
					</p>
					...
					<p>
						...
						<span>
							Modest membership dues provide the financial support needed to{' '}
						</span>
						<span>
							maintain this resource as a freely-available service to the{' '}
						</span>
						<span>community.</span>
					</p>
				</>
			...
			{!tag && auth.user?.admin && (
				<Link
					to={`/openev/${year || startOfYear()}/upload`}
