...
describe('FilesTable', () => {
    it('Renders a files table', async () => {
        const files = [
            { openev_id: 1, name: 'Test.docx', path: '/test', year: startOfYear, camp: 'CNDI', tags: '{"da":true,"cp":true}' },
        ];
        render(<FilesTable files={files} />);
        assert.isOk(screen.queryByText(/Test.docx/), 'File link exists');
        assert.isOk(document.querySelector('.download'), 'Download link exists');
        ...
        await waitFor(() => assert.isOk(screen.queryByText(/Confirm/), 'Confirm prompt exists'));
        fireEvent.click(screen.queryByText(/Confirm/));
        await waitFor(() => assert.strictEqual(deleteOpenEvFile.mock.calls.length, 1, 'Deleted openev files'));
        await waitFor(() => assert.strictEqual(store.fetchOpenEvFiles.mock.calls.length, 1, 'Fetched openev files'));
    });
    ...
    it('Only renders delete icon for admins', async () => {
        ...
        render(<FilesTable files={files} />);
        assert.isNotOk(document.querySelector('.trash'), 'No Trash button exists for non-admin');
