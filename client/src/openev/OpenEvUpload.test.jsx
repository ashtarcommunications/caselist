...
describe('Open Ev Upload', () => {
	it('Renders and submits an upload form', async () => {
		render(<OpenEvUpload />, {
			route: '/openev/:year',
			path: `/openev/${startOfYear()}`,
		});
		await waitFor(() =>
			assert.isOk(screen.queryByText(/Upload a file/), 'Heading exists'),
		);
		...
		await waitFor(() =>
			assert.isOk(
				screen.queryByText(/letters and numbers/),
		...
		await waitFor(() =>
			assert.isOk(screen.queryByText(/all caps/), 'All Caps warning exists'),
		);
		fireEvent.change(lab, { target: { value: 'AB' } });
		assert.strictEqual(lab.value, 'AB', 'Correct lab input value');

		await waitFor(() =>
			assert.isOk(screen.queryByText(/Affirmatives/), 'Tag checkboxes exist'),
		);
		...
		const renamed = new RegExp(
			`Test Title - CNDI ${startOfYear()} AB.docx`,
			'g',
		);
		await waitFor(() =>
			assert.isOk(screen.queryByText(renamed), 'Renamed filename'),
		);
		...
		await waitFor(() =>
			assert.strictEqual(
				addOpenEvFile.mock.calls.length,
				1,
				'Submitted the form',
			),
		);
		await waitFor(() =>
			assert.isOk(
				screen.queryByText(/Successfully added file/),
				'Success notification',
			),
		);
		await waitFor(() =>
			assert.isNotOk(screen.queryByText(/test.docx/), 'Uploaded file reset'),
		);
		await waitFor(() =>
			assert.isOk(screen.queryByText(/Drag and drop/), 'Dropzone appears'),
		);
	});

	it('Displays an error message on failure', async () => {
		addOpenEvFile.mockRejectedValue({ message: 'Failed to upload file' });
		...
		fireEvent.change(document.querySelector('input[name="lab"]'), {
			target: { value: 'AB' },
		});
		...
		await waitFor(() =>
			assert.isOk(
				screen.queryByText(/Failed to add file/),
